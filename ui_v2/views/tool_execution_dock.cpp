...
void ToolExecutionDock::exportData(const QString& format) {
    QString filter;
    QString extension;
    ...
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Export Execution Data"),
        QString("tool_executions.%1").arg(extension),
        filter
    );
    ...
    if (!fileName.isEmpty()) {
        ...
        if (format == "json") {
            ...
            for (const ToolExecution& exec : executions_) {
                QJsonObject obj;
                obj["id"] = exec.id.toString();
                obj["toolName"] = exec.toolName;
                obj["description"] = exec.description;
                obj["parameters"] = exec.parameters;
                obj["startTime"] = exec.startTime.toString(Qt::ISODate);
                obj["endTime"] = exec.endTime.toString(Qt::ISODate);
                obj["duration"] = static_cast<qint64>(exec.duration);
                obj["status"] = static_cast<int>(exec.status);
                obj["progress"] = exec.progress;
                obj["output"] = exec.output;
                obj["errorMessage"] = exec.errorMessage;
                obj["metrics"] = exec.metrics;
                array.append(obj);
            }
            ...
            // CSV header
            stream << "Tool,Status,Start Time,End Time,Duration (ms),Progress,Description\n";
            
            for (const ToolExecution& exec : executions_) {
                QString status;
                switch (exec.status) {
                case ToolExecution::Pending: status = "Pending"; break;
                case ToolExecution::Running: status = "Running"; break;
                case ToolExecution::Success: status = "Success"; break;
                case ToolExecution::Failed: status = "Failed"; break;
                case ToolExecution::Cancelled: status = "Cancelled"; break;
                }
                
                stream << exec.toolName << ","
                      << status << ","
                      << exec.startTime.toString(Qt::ISODate) << ","
                      << exec.endTime.toString(Qt::ISODate) << ","
                      << exec.duration << ","
                      << exec.progress << ","
                      << "\"" << exec.description.replace("\"", "\"\"") << "\"\n";
            }
        }
...
void ToolExecutionDock::exportMetrics(const QString& format) {
    // Export aggregated metrics
    QString fileName = QFileDialog::getSaveFileName(
        this, tr("Export Metrics"),
        QString("tool_metrics.%1").arg(format),
        tr("%1 Files (*.%2)").arg(format.toUpper()).arg(format)
    );
    ...
    if (!fileName.isEmpty()) {
        ...
        if (format == "csv") {
            stream << "Tool,Executions,Success,Failed,Success Rate,Avg Duration (ms)\n";
            ...
            for (auto it = tools_.begin(); it != tools_.end(); ++it) {
                ...
                if (info.executionCount > 0) {
                    successRate = (info.successCount * 100.0) / info.executionCount;
                }
                
                qint64 avgDuration = 0;
                if (info.executionCount > 0) {
                    avgDuration = info.totalDuration / info.executionCount;
                }
