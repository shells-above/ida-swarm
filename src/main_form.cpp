...
void MainForm::setup_agent() {
    ...
    REAgent::Config agent_config;
    agent_config.model = config_->api.model;
    agent_config.max_iterations = config_->agent.max_iterations;
    agent_config.max_tokens = config_->api.max_tokens;
    agent_config.temperature = config_->api.temperature;
    agent_config.enable_thinking = config_->agent.enable_thinking;
    agent_config.enable_prompt_caching = config_->api.enable_prompt_caching;

    // Create agent
    agent_ = std::make_unique<REAgent>(config_->api.api_key, agent_config);
...
bool Config::save_to_file(const std::string& path) const {
    try {
        ...
        j["api"]["timeout_seconds"] = api.timeout_seconds;
...
bool Config::load_from_file(const std::string& path) {
    try {
        ...
        if (j.contains("api")) {
            ...
            api.timeout_seconds = j["api"].value("timeout_seconds", api.timeout_seconds);
        }
