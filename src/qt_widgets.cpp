...
// MemoryMapWidget implementation
MemoryMapWidget::MemoryMapWidget(QWidget* parent) : QWidget(parent) {
    setMinimumHeight(200);
    setMouseTracking(true);
}

void MemoryMapWidget::update_memory(const json& memory_snapshot) {
    regions.clear();

    // Process functions from memory snapshot
    if (memory_snapshot.contains("functions")) {
        std::map<std::string, MemoryRegion> segment_map;

        for (const auto& func : memory_snapshot["functions"]) {
            // Parse address from hex string
            std::string addr_str = func["address"].get<std::string>();
            ea_t addr = std::stoull(addr_str, nullptr, 16);

            std::string segment = "code";  // Default to code segment for functions

            auto& region = segment_map[segment];
            if (region.name.empty()) {
                region.name = segment;
                region.start = addr;
                region.end = addr;
                region.color = QColor(100, 150, 255);  // Blue for code
            } else {
                region.start = std::min(region.start, addr);
                region.end = std::max(region.end, addr);
            }
        }

        // Convert map to vector
        for (const auto& [name, region] : segment_map) {
...
void MemoryMapWidget::highlight_address(ea_t address) {
    highlighted_addresses.insert(address);
    update();
}

void MemoryMapWidget::clear_highlights() {
...
void MemoryMapWidget::paintEvent(QPaintEvent* event) {
    ...
    if (regions.empty()) {
        painter.drawText(rect(), Qt::AlignCenter, "No memory data available");
        return;
    }

    // Find min/max addresses
    ea_t min_addr = std::numeric_limits<ea_t>::max();
    ea_t max_addr = 0;
    ...
    int region_height = 40;
    int y_offset = margin;

    for (const auto& region : regions) {
        QRect rect = region_rect(region);

        // Draw region
        painter.fillRect(rect, region.color);
        painter.setPen(Qt::black);
        painter.drawRect(rect);

        // Draw label
        painter.drawText(rect, Qt::AlignCenter, QString::fromStdString(region.name));

        // Draw address range
        QString addr_text = QString("0x%1 - 0x%2")
            .arg(region.start, 0, 16)
            .arg(region.end, 0, 16);
        painter.drawText(rect.adjusted(5, -15, -5, 0), Qt::AlignTop, addr_text);

        // Highlight specific addresses
        painter.save();
        for (ea_t addr : highlighted_addresses) {
            if (addr >= region.start && addr <= region.end) {
                double pos = (double)(addr - region.start) / (region.end - region.start);
                int x = rect.left() + pos * rect.width();
    ...
    painter.drawText(margin, height() - 5, QString("0x%1").arg(min_addr, 0, 16));
    painter.drawText(width() - margin - 60, height() - 5, QString("0x%1").arg(max_addr, 0, 16));
}

void MemoryMapWidget::mousePressEvent(QMouseEvent* event) {
    ea_t addr = address_at_point(event->pos());
    if (addr != BADADDR) {
        emit address_clicked(addr);
    }
}

void MemoryMapWidget::mouseDoubleClickEvent(QMouseEvent* event) {
    ea_t addr = address_at_point(event->pos());
    if (addr != BADADDR) {
        emit address_double_clicked(addr);
    }
}

ea_t MemoryMapWidget::address_at_point(const QPoint& point) {
    int margin = 20;

    for (const auto& region : regions) {
        QRect rect = region_rect(region);
        if (rect.contains(point)) {
            // Calculate address from x position
            double pos = (double)(point.x() - rect.left()) / rect.width();
            ea_t addr = region.start + pos * (region.end - region.start);
            return addr;
        }
    }
    return BADADDR;
...
QRect MemoryMapWidget::region_rect(const MemoryRegion& region) {
    // Find min/max for scaling
    ea_t min_addr = std::numeric_limits<ea_t>::max();
    ea_t max_addr = 0;
    for (const auto& r : regions) {
        min_addr = std::min(min_addr, r.start);
        max_addr = std::max(max_addr, r.end);
    }
    ...
    // Calculate position
    double start_pos = (double)(region.start - min_addr) / (max_addr - min_addr);
    double end_pos = (double)(region.end - min_addr) / (max_addr - min_addr);

    int x = margin + start_pos * available_width;
    int w = std::max(50, (int)((end_pos - start_pos) * available_width));
    int y = y_offset + index * (region_height + 10);

    return QRect(x, y, w, region_height);
}
        ...
        if (item->text(1) == "Running..." || item->text(0) == QString::fromStdString(tool_id)) {
            ...
            if (result.contains("success") && result["success"].get<bool>()) {
                ...
                if (result.contains("error")) {
                    ...
                    error_item->setForeground(1, QColor(ColorScheme::ERROR));
                }
            }
